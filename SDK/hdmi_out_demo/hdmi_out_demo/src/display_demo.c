...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
		...
		case '6':
			...
			break;
                case '7':
			DemoBlankFrame(dispCtrl.framePtr[dispCtrl.curFrame], dispCtrl.vMode.width, dispCtrl.vMode.height, dispCtrl.stride);
			break;
		case 'q':
			break;
		default :
...
void DemoBlankFrame(u8 *srcFrame, u32 width, u32 height, u32 stride)
{
	u32 xcoi, ycoi;
	u32 lineStart = 0;
	for(ycoi = 0; ycoi < height; ycoi++)
	{
		for(xcoi = 0; xcoi < (width * 3); xcoi+=3)
		{
			if (xcoi < 512 && ycoi < 128)
				srcFrame[xcoi + lineStart] = xcoi; //Blue
			else
			    srcFrame[xcoi + lineStart] = 0; //Blue

			srcFrame[xcoi + lineStart + 1] = 0; //Green

			if (xcoi < 256 && ycoi < 256)
				srcFrame[xcoi + lineStart + 2] = xcoi; //Red
			else
			    srcFrame[xcoi + lineStart + 2] = 0; //Red

		}
		lineStart += stride;
	}

	xil_printf("Stride: %d", stride);
